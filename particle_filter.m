function [xhk, pf, yhk, yh_5, yh_95] = particle_filter(sys, yk, pf, resampling_strategy)
...
yhat_k = zeros(Ns,1);
for i = 1:Ns;
   xhk = xhk + wk(i)*xk(:,i); % This is the mean value
   x_sample_i = xk(:,i);
   yhat_k(i) = x_sample_i(1).*exp(x_sample_i(2).*k) + x_sample_i(3).*exp(x_sample_i(4).*k); % Estimate y
end
[yhat_k_sort, I] = sort(yhat_k);
w_k_sort = wk(I);
yhat_k_cdf = cumsum(w_k_sort);
index_5 = find(yhat_k_cdf>.05,1);
index_95 = find(yhat_k_cdf>.95,1);
yhk = yhat_k'*wk;
yh_5 = yhat_k_sort(index_5);
yh_95 = yhat_k_sort(index_95);
